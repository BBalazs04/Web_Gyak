...
// Adatbázis kapcsolat (ugyanaz, mint korábban)
...
$sql = "SELECT nev, email, targy, uzenet, bekuldes_ido FROM uzenetek ORDER BY bekuldes_ido DESC";
...
<body>
    ...
    <h1 style="text-align:center;">Elküldött üzenetek</h1>
        ...
        <table>
            ...
            <tbody>
                <?php while($row = $result->fetch_assoc()): ?>
                    <tr>
                        <td><?= htmlspecialchars($row['bekuldes_ido']) ?></td>
                        <td>
                            <?php
                                $nev = trim($row['nev']);
                                echo $nev === '' ? 'Vendég' : htmlspecialchars($nev);
                            ?>
                        </td>
    ...
    <?php else: ?>
        <p style="text-align:center;">Nincs megjeleníthető üzenet.</p>
    <?php endif; ?>
