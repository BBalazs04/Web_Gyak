...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($username !== '' && $password !== '' && $Vezeteknev !== '' && $Keresztnev !== '') {
        $hashedPassword = password_hash($password, PASSWORD_DEFAULT);

        $stmt = $pdo->prepare("INSERT INTO users (username, password, Vezeteknev, Keresztnev) VALUES (?, ?, ?, ?)");
        try {
            $stmt->execute([$username, $hashedPassword, $Vezeteknev, $Keresztnev]);
